...
class Node(
    ...
    /** The file holding layer [i] for this runtime: layer_07.npz for the CPU engine, layer_07.tflite
     *  for the NPU engine. The hub serves either from the same /shards route. */
    private fun layerFile(i: Int) = "layer_%02d.%s".format(i, engine.shardExt)

    /** Layer ids already on disk, so the planner knows what a move to this phone would cost. Only
     *  this runtime's own shards count: a tflite is no use to the CPU engine, or the other way round. */
