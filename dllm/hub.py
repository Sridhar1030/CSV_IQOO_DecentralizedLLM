...
def _layer_ids(files) -> set[int]:
    """Layer ids a node already holds. Either shard kind counts: the Kotlin CPU engine downloads
    layer_07.npz, the Hexagon NPU engine layer_07.tflite, and the planner prices a move by how many
    of the files it would need are already there."""
    return {int(m.group(1)) for f in files for m in [re.fullmatch(r"layer_(\d+)\.(?:npz|tflite)", f)] if m}
